...
if ($players >0) {
					...
					foreach ($player_list as $k=>$v) {
						...
						// add sub template
							...
							$disp .='<tr><td><i style="color:green;">'.$playerN.'</i></td><td style="margin:auto;">'.$result['flag'].'</td><td>'.$pscore.'</td><td style="padding-left:1%;">'.gmdate("H:i:s", $player_list[$k]['gq_time']).'</td></tr>';
							//print_r($result);
						}
						else {
