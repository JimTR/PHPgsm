...
if ($players >0) {
					// we have players
					// add sub template
					$disp .= '<table style="width:100%;"><tr><td style="width:20%;">Name</td><td style="width:20%;">Country</td><td style="width:20%;">Score</td><td>Time Online</td></tr>'; // start table
					...
					foreach ($player_list as $k=>$v) {
						...
						// add sub template
							...
							$disp .='<tr><td><i style="color:green;">'.$playerN.'</i></td><td align="center">'.$result['flag'].' '.$result['country'].'</td><td>'.$pscore.'</td><td style="padding-left:1%;">'.gmdate("H:i:s", $player_list[$k]['gq_time']).'</td></tr>';
							//print_r($result);
						}
						else {
