...
if ($info['Players'] >0) {
					...
					foreach ($player_list as $k=>$v) {
						...
						// add sub template
							...
							$disp .='<tr class="country"><td><i class="player_n" style="">'.$playerN.'</i></td><td>'.$map.' <span class="country">'.$result['country'].'</span></td><td style="text-align:right;padding-right:5%;">'.$pscore.'</td><td style="text-align:right;">'. $player_list[$k]['TimeF'].'</td></tr>';
							//print_r($result);
						}
						else {
									$disp .='<tr class="country"><td><i class="player_n" style="" >'.$playerN.'</i></td><td></td><td style="text-align:right;padding-right:5%;">'.$pscore.'</td><td style="text-align:right;">'. $player_list[$k]['TimeF'].'</td></tr>';
						}
