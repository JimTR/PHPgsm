...
foreach ($info as $k =>$game) {
	...
	foreach ($game as $k1=>$record) {
		...
		if($k1 <> 'general') {
			...
			$track->addChild('version',$record['buildid'].' (last updated '.date('l jS F Y \a\t g:ia',floatval($record['server_update'])).')');
...
foreach ($bases as $data) {
		...
		$track->addChild('quota_pc',number_format( floatval($info[$fn]['general']['total_size'])/floatval($sdata['quota']) *1000,2));
