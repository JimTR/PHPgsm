...
switch (strtolower($cmds['action'])) {
	...
	case "lsof" :
					...
					if (isset($cmds['lsof_file'])) {
						...
						$tmp = shell_exec('lsof 2> /dev/null | grep -m1 '.$cmds['lsof_file']);
		...
		$ip = file_get_contents("http://ipecho.net/plain"); // get ip
		 ...
		 $server_data = $db->get_row($sql);
		  ...
		  if (empty($server_data['base_ip'])) {         
                ...
                if (empty($new)) {
		// offline
		if (!empty($server_data['location'])) { file_put_contents($server_data['location'].cr,'loc.txt',FILE_APPEND);}
